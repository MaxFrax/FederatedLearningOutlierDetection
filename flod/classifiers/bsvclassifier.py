...
class BSVClassifier(ClassifierMixin, BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        self.betas_, self.constant_term_ = BSVClassifier._solve_optimization_tensorflow(X, y, self.random_seed, self.init_bound, self.c, self.q, self.n_iter, self.optimizer, self.penalization)
    ...
    def _solve_optimization_tensorflow(xs, y, random_seed, init_bound, c, q, n_iter, optimizer, penalization):
        ...
        if abs(init_bound) > abs(c):
            LOGGER.warning(f'init_bound cannot be bigger than c. Defaulting to init_bound=c')
            init_bound = c
        
        betas = []
        
        LOGGER.debug(f'Optimizing with q {q}')
        gaussian_kernel = lambda x1, x2: BSVClassifier._gaussian_kernel(x1, x2, q)
        ...
        for i in range(len(xs)):
            ...
            beta = tf.Variable(init, name=f'beta_{i}', trainable=True, dtype=tf.float64)
            betas.append(beta)

        self_kernels = [gaussian_kernel(x_i, x_i) for x_i in xs]
        kernel_values = np.array([[gaussian_kernel(x1, x2) for x1 in xs] for x2 in xs])
        kernels = tf.constant(kernel_values, dtype='float64')

        def objective_function():
            constant_term = tf.tensordot(tf.linalg.matvec(kernels, betas), betas, axes=1)
        ...
        LOGGER.info(f'Optimized in {tot_iter} iterations. Latest update size: {last_update}')

        constant_term = tf.tensordot(tf.linalg.matvec(kernels, best_betas), best_betas, axes=1)
    ...
    def _compute_r(self, x) -> float:
        ...
        v += -2 * tf.tensordot(self.betas_, [self._gaussian_kernel(x_i, x, self.q) for x_i in self.X_], axes=1)
        v = np.sqrt(v)
        return v
